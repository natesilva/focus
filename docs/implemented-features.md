...
**Suggested approach when revisiting (original notes):** Mount at `/focus/<dirname>` using nested mounts — a per-project named volume at `/focus` (persists across restarts) with the project bind-mount layered on top at `/focus/<dirname>`. The named volume must be declared before the bind-mount. Run `git worktree prune` on container start to clean up stale metadata. Key the named volume to the project path hash so different projects don't share a namespace.

**What was built:** Both Docker and Apple Containers adapters now mount `focus-ws-<hash8>:/focus` (named volume) followed by `<cwd>:/focus/<dirname>` (bind-mount). `StartOptions` gained a `workspaceVolume` field; `workspaceVolumeName()` in `container.ts` derives the volume name from the same 8-char sha256 prefix as `containerName()`. `configHash()` includes `layoutVersion: 2` so pre-existing containers (mounted at the old `/focus` path) are detected as stale and rebuilt. `attachContainer()` passes `/focus/<dirname>` as the exec workdir. `entrypoint.sh` `cd`s to `/focus/$FOCUS_PROJECT` and runs `git worktree prune` (silenced, non-fatal) on each start.
