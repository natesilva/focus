...
Defines the per-project named volume that provides persistent storage at `/focus` inside the container, enabling worktree and tool state to survive container restarts while keeping the project bind-mount layered at `/focus/<dirname>`.
